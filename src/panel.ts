...
import { ParseResult, resolveVariableMatrix, EnvironmentMatrix } from './variableParser';

export class VariableInspectorPanel {
  public static currentPanel: VariableInspectorPanel | undefined;
  private static readonly viewType = 'adoVarInspector';
  ...
  public static createOrShow(
    ...
    const panel = vscode.window.createWebviewPanel(
      VariableInspectorPanel.viewType,
  ...
  private _getHtmlContent(parseResult: ParseResult, filename: string): string {
    ...
    // Build matrix data
    const matrices: EnvironmentMatrix[] = [];
    for (const [name, entries] of variables.entries()) {
      matrices.push(resolveVariableMatrix(name, entries, envColumns));
    }
    ...
    const tableRows = matrices.map(m => {
      ...
      const cells = envColumns.map(env => {
        ...
        if (val === undefined) return `<td class="cell-undefined"><span class="undef">—</span></td>`;
    ...
    const envHeaders = envColumns.map(e =>
      `<th class="col-env">${escapeHtml(e)}</th>`
    ).join('');
